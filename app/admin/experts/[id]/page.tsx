...
export default function ExpertReviewPage() {
  ...
  const handleUpdateStatus = async (status: string) => {
    ...
    setIsUpdating(true)
    try {
      // Build tags payload from editedTags state
      const tagsPayload: { tagType: string; tagValue: string }[] = []
      ;(Object.keys(editedTags) as Array<keyof TagGroup>).forEach((tagType) => {
        editedTags[tagType].forEach((tagValue) => {
          tagsPayload.push({ tagType, tagValue })
        })
      })
      ...
      const res = await fetch(`/api/admin/experts/${expertId}`, {
        ...
        body: JSON.stringify({ status, notes: reviewNotes, tags: tagsPayload }),
      })
      if (res.ok) {
