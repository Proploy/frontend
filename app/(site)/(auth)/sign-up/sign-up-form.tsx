...
/**
 * `redirectTo` is resolved on the server (see page.tsx) and already checked to
 * be a relative path. It was previously re-read from `window.location` at
 * submit time, which worked but left the sign-in link below unable to carry it.
 */
export function SignUpForm({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'password') {
      setErrors({
        ...
        specialChar: !SPECIAL_CHAR.test(value),
      })
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push(withRedirect('/check-email', redirectTo, { email: formData.email }))
    } catch (err) {
  ...
  const handleOAuth = async (provider: OAuthProvider) => {
    ...
    try {
      const { url, error } = await startOAuthSignIn({ provider, redirectTo: redirectTo ?? undefined })
      if (error) throw error
  ...
  const passwordChecks = [
    ...
    { ok: SPECIAL_CHAR.test(formData.password), label: 'At least one special character' },
  ]
          ...
          <ul className="pp-stack pp-gap-2" aria-live="polite">
            {passwordChecks.map(({ ok, label }) => (
              <li key={label} className="pp-flex pp-gap-2" style={{ alignItems: 'center' }}>
                ...
                <span className="pp-small" style={ok ? { color: 'var(--ink)' } : undefined}>
                  {label}
                </span>
              </li>
            ))}
