...
/**
 * `redirectTo` and `errorParam` are resolved on the server (see page.tsx) and
 * passed down, rather than read here with `useSearchParams`. Reading them in
 * the client would opt the whole route out of static rendering and force the
 * form behind a Suspense skeleton on first paint.
 *
 * `redirectTo` has already been checked to be a relative path, so it is safe
