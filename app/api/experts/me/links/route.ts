...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.linkType || !body.url) {
      return createErrorResponse('VALIDATION_ERROR', 'linkType and url required', 400)
    }
    const res = await serviceApisFetch('/api/v1/experts/me/links', {
      method: 'POST',
      requireAuth: true,
      body: JSON.stringify({ linkType: body.linkType, url: body.url }),
    })
