      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="pf-pagebar">
          <div className="flex min-w-0 items-center gap-[14px]">
            <span className="pf-ico pf-ico--lg pf-ico--soft"><FileText size={20} /></span>
            <div className="pf-pagebar-text">
              <span className="pf-eyebrow">Workspace</span>
              <h1 className="pf-title truncate">Contracts</h1>
            </div>
          </div>
          <button
            type="button"
          ...
          >
            <RefreshCw size={16} className={loading ? 'animate-spin text-cobalt' : 'text-cobalt'} />
            Refresh
          </button>
        </header>
        ...
        <div className="flex min-h-0 flex-1 flex-col xl:flex-row">
          <section className="flex flex-col border-b border-line bg-white xl:w-[420px] xl:shrink-0 xl:border-b-0 xl:border-r">
            <div className="border-b border-line px-[16px] py-[14px]">
              <h2 className="pf-h2">All contracts</h2>
            ...
            <div className="flex flex-1 flex-col gap-[4px] overflow-y-auto p-[8px]">
              {sorted.length === 0 && !loading ? <p className="px-[12px] py-[24px] text-center text-[14px] leading-[20px] text-ink-muted">Contracts appear after a proposal is accepted.</p> : null}
              {sorted.map((contract) => (
                <button
                  key={contract.id}
                  type="button"
                  onClick={() => setSelectedId(contract.id)}
                  className={`rounded-[10px] border p-[12px] text-left transition-colors ${selected?.id === contract.id ? 'border-cobalt bg-cobalt-soft' : 'border-transparent hover:bg-surface-hover'}`}
                >
                  <div className="flex items-start justify-between gap-[8px]">
                    <span className="min-w-0">
                      <span className="block truncate text-[14px] font-semibold leading-[20px] text-ink">{contract.title}</span>
                      <span className="mt-[2px] block truncate text-[13px] leading-[18px] text-ink-soft">{engagementMap.get(contract.engagementId) ? engagementTitle(engagementMap.get(contract.engagementId)!, state.role) : 'Engagement'}</span>
                    </span>
                  ...
                  <p className="mt-[8px] text-[12px] leading-[18px] text-ink-muted">Updated {relativeDate(contract.updatedAt)}</p>
                </button>
              ))}
            </div>
          </section>
            ...
            {selected ? (
              ...
              <div className="flex h-full items-center justify-center">
                <div className="max-w-[360px] text-center">
                  <FileText size={32} className="mx-auto text-line" />
                  <h2 className="pf-h2 mt-[12px]">No contract selected</h2>
                  <p className="mt-[4px] text-[14px] leading-[20px] text-ink-soft">Accepting a proposal creates the first contract draft.</p>
                </div>
              </div>
            )}
          </section>
...
function ContractDetail({
  ...
  return (
    <div className="mx-auto flex max-w-[820px] flex-col gap-[16px]">
      <article className="pf-card">
        <div className="border-b border-line px-[32px] pb-[24px] pt-[32px]">
          <div className="flex flex-wrap items-center justify-between gap-[12px]">
            ...
            <span className="text-[12px] font-medium text-ink-muted">Updated {relativeDate(contract.updatedAt)}</span>
          </div>
          <h2 className="pf-title mt-[16px]">{contract.title}</h2>
          <p className="mt-[4px] text-[14px] leading-[20px] text-ink-soft">Engagement {engagementLabel}</p>
        </div>
        ...
        <div className="px-[32px] py-[24px]">
          ...
          <div className="mt-[16px] flex flex-col gap-[12px]">
            {displaySections.map((item) => (
              <div key={item.title} className="rounded-[10px] border border-line bg-white px-[16px] py-[14px]">
                <h3 className="text-[13px] font-semibold uppercase tracking-[0.04em] text-cobalt">{item.title}</h3>
                <div className="mt-[8px] flex flex-col gap-[6px] text-[14px] leading-[22px] text-ink-soft">
                  {item.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                  {item.bullets.map((bullet) => <p key={bullet} className="flex gap-[8px]"><span className="text-cobalt">-</span><span>{bullet}</span></p>)}
                </div>
              </div>
            ))}
          </div>
          ...
          {isExpert && (contract.status === 'draft' || contract.status === 'declined') ? (
            <div className="mt-[20px] rounded-[12px] border border-cobalt-soft bg-cobalt-soft p-[16px]">
              <div className="flex flex-wrap items-start justify-between gap-[10px]">
                <div>
                  <h3 className="pf-h2">{contract.status === 'declined' ? 'Revise and resubmit contract' : 'Complete contract details'}</h3>
                  <p className="mt-[3px] text-[13px] leading-[20px] text-ink-soft">Fill each field before sending this agreement to the buyer. These values are also used in the final signed PDF.</p>
                </div>
                <span className="rounded-full bg-cobalt-soft px-[9px] py-[3px] text-[11px] font-semibold text-cobalt-deep">Draft editor</span>
              </div>
              <div className="mt-[14px] grid gap-[12px] md:grid-cols-2">
                {CONTRACT_FIELDS.map((field) => (
                  <label key={field.key} className={`flex flex-col gap-[6px] ${field.type === 'textarea' ? 'md:col-span-2' : ''}`}>
                    ...
                    {field.type === 'textarea' ? (
                      <textarea value={fieldValues[field.key]} onChange={(event) => setField(field.key, event.target.value)} rows={3} placeholder={`Enter ${field.label.toLowerCase()}`} className="resize-y rounded-[8px] border border-line bg-white px-[11px] py-[9px] text-[13px] leading-[20px] text-ink placeholder:text-ink-faint focus:outline-none focus:ring-2 focus:ring-cobalt/30" />
                    ) : (
                      <input type={field.type} value={fieldValues[field.key]} onChange={(event) => setField(field.key, event.target.value)} placeholder={`Enter ${field.label.toLowerCase()}`} className="rounded-[8px] border border-line bg-white px-[11px] py-[9px] text-[13px] leading-[20px] text-ink placeholder:text-ink-faint focus:outline-none focus:ring-2 focus:ring-cobalt/30" />
                    )}
                  </label>
                ))}
              </div>
              <div className="mt-[14px] flex flex-wrap items-center justify-between gap-[10px]">
                <p className="text-[12px] leading-[18px] text-ink-muted">Saving replaces the placeholders in the stored agreement. After saving, use the same send button to resubmit a declined contract.</p>
                <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onUpdate(fieldValues) }} disabled={busy || !contractContentCompleteness(fieldValues).ok} className="pf-btn pf-btn--primary pf-btn--sm">
      ...
      <div className="flex flex-col gap-[14px] rounded-[12px] border border-line bg-white p-[16px]">
        <div className="flex items-start gap-[8px] text-[13px] leading-[18px] text-ink-soft">
          <CheckCircle2 size={16} className="mt-[1px] shrink-0 text-ok" />
          <span>Both parties must sign. The buyer signs first, then the expert countersigns.</span>
        </div>
        {contract.status === 'buyer_signed' ? (
          <div className="rounded-[8px] border border-cobalt-soft bg-cobalt-soft px-[12px] py-[10px] text-[13px] leading-[18px] text-cobalt-deep">
        ...
        {signingRole && signerAction === 'waiting' ? (
          <div className="rounded-[8px] border border-cobalt-soft bg-cobalt-soft px-[12px] py-[11px] text-[13px] leading-[19px] text-cobalt-deep">
        ...
        {contract.signedDocumentAvailable ? (
          <div className="flex flex-wrap items-center justify-between gap-[10px] rounded-[8px] border border-ok-line bg-ok-soft px-[12px] py-[10px] text-[13px] text-ok">
            <span>
            ...
            <div className="flex items-center gap-[8px]">
              {signedDocumentUrl ? (
                <button type="button" onClick={onOpenSignedDocument} className="pf-btn pf-btn--secondary pf-btn--sm">
                  <Eye size={15} /> Open signed document
                </button>
              ) : (
                <button type="button" onClick={onLoadSignedDocument} disabled={documentLoading} className="pf-btn pf-btn--secondary pf-btn--sm">
        ...
        {isExpert && (contract.status === 'draft' || contract.status === 'declined') ? (
          <div className="flex flex-col gap-[10px]">
            {!savedCompleteness.ok ? (
              <div className="rounded-[8px] border border-warn-line bg-warn-soft px-[12px] py-[10px] text-[13px] leading-[19px] text-warn">
                Complete and save the required fields before sending: {savedCompleteness.missing.join(', ')}.
              </div>
            ) : null}
            <div className="flex flex-wrap justify-end gap-[8px]">
            <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onCancel() }} disabled={busy} className="pf-btn pf-btn--secondary">
              <XCircle size={17} /> Cancel
            </button>
            <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onSend() }} disabled={busy || !savedCompleteness.ok} className="pf-btn pf-btn--primary">
        ...
        {isExpert && contract.status === 'sent' ? (
          <div className="flex flex-wrap justify-end gap-[8px]">
            ...
            <span className="self-center text-[13px] text-ink-muted">Waiting for the buyer to sign.</span>
          </div>
        ) : null}
        {isBuyer && contract.status === 'sent' ? (
          <div className="flex flex-wrap justify-end gap-[8px] border-t border-line pt-[14px]">
            <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onDecline() }} disabled={busy} className="pf-btn pf-btn--danger">
        ...
        {canSign ? (
          <div className="flex flex-col gap-[10px] border-t border-line pt-[14px]">
            <div className="rounded-[8px] border border-ok-line bg-ok-soft px-[12px] py-[10px] text-[13px] font-semibold leading-[19px] text-ok">
              {signingRole === 'buyer' ? 'Your signature is required first.' : 'The buyer has signed. Your countersignature is required.'}
            </div>
            <label className="flex flex-col gap-[6px]">
              <span className="text-[13px] font-semibold leading-[18px] text-ink-soft">Typed signature name</span>
              <input value={signatureName} onChange={(event) => onSignatureNameChange(event.target.value)} placeholder="Your full legal name" className="w-full rounded-[8px] border border-line bg-white px-[12px] py-[10px] text-[14px] leading-[20px] text-ink placeholder:text-ink-muted focus:outline-none focus:ring-2 focus:ring-cobalt/30" />
            </label>
            <div className="flex justify-end">
              <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onSign() }} disabled={busy || !signatureName.trim()} className="pf-btn pf-btn--primary">
                <PenLine size={17} /> {busyAction === 'sign' ? 'Signing…' : `Sign as ${signingRole}`}
              </button>
            </div>
          </div>
        ) : null}
        {contract.status === 'completed' ? <p className="flex items-center gap-[8px] text-[13px] font-semibold text-ok"><CheckCircle2 size={16} /> Fully signed and completed.</p> : null}
      </div>
          ...
          >
            <div className="flex items-center justify-between gap-[12px] border-b border-line px-[16px] py-[12px] sm:px-[20px]">
              <div>
                ...
                <p className="mt-[2px] text-[12px] text-ink-muted">This document is available only to the parties on this engagement.</p>
              </div>
              <button
                type="button"
                onClick={onCloseSignedDocument}
            ...
            <div className="min-h-0 flex-1 bg-line-soft p-[8px] sm:p-[16px]">
              <iframe
                ...
                className="h-full w-full rounded-[8px] border border-line bg-white"
              />
            </div>
          </div>
        </div>
