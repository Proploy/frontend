...
export function SchedulingProviders() {
  ...
  return (
    <section className={`overflow-hidden rounded-[12px] border border-line bg-white ${CARD_SHADOW}`}>
      <div className="flex flex-col gap-[2px] border-b border-line px-[20px] py-[16px]">
        <div className="flex items-center gap-[8px]">
          <h2 className="font-semibold text-[16px] leading-[24px] text-ink">Scheduling</h2>
          <span className="rounded-full bg-cobalt-soft px-[8px] py-[1px] text-[12px] font-medium text-cobalt">
            Cal.com
          </span>
        </div>
          ...
          return (
            <div
              ...
              className={[
                ...
                active ? 'border-cobalt bg-cobalt-soft' : 'border-line bg-white',
              ].join(' ')}
            >
              <div className="flex items-start gap-[10px]">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-[6px]">
                    <p className="font-semibold text-[14px] leading-[20px] text-ink">{def.name}</p>
                    {active && (
                      <span className="rounded-full bg-cobalt px-[7px] py-[1px] text-[11px] font-semibold leading-[16px] text-white">
                        Active
                      </span>
                    )}
                  </div>
                  <p className="truncate text-[12px] leading-[18px] text-ink-muted">
              ...
              {connected ? (
                <button
                  ...
                  className={[
                    ...
                    active
                      ? 'cursor-default border border-line bg-white text-ink-muted'
                      : `border border-line bg-white text-ink-soft hover:bg-surface-hover ${BUTTON_SKEUO}`,
                  ].join(' ')}
                >
                  {active ? (
                    <>
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-[6px] rounded-[8px] border border-line bg-white px-[12px] py-[7px] text-[13px] font-semibold leading-[18px] text-ink-soft hover:bg-surface-hover ${BUTTON_SKEUO}`}
                >
